...
# 讀取結果文件
def load_calibration_result(file_path):
    try:
...
updo_result = load_calibration_result('/home/robot/wheeltec_ros2/src/rs485_fire/src/calibration_updo.json')
if updo_result:
...
class MotorControllerupdo:
    ...
    def zero_calibration(self):
        ...
        while True:
            self.move_motor(0x0001, 1.0, 250)  # 嘗試大範圍移動
        ...
        stable_count, previous_encoder = 0, None
        while True:
            self.move_motor(0x0000, 1.0, 250)  # 嘗試大範圍移動
            current_encoder = self.read_encoder_raw()
    ...
    def move_to_angle(self, zero_encoder, target_angle):
        ...
        try:
            ...
            self.move_motor(direction, target_circle, 200)
...
if __name__ == "__main__":
    ...
    if not motor.connect():
        ...
        try:
            # 載入校準結果
            calibration_result = load_calibration_result('/home/robot/wheeltec_ros2/src/rs485_fire/src/calibration_updo.json')
